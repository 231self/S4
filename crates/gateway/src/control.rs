...
pub struct UsageEvent {
    /// Receipt idempotency key. Client-supplied only through the authenticated
    /// `x-s4-metering-id` protocol header; otherwise generated by the gateway.
    pub id: Uuid,
...
pub trait ControlPlane: Send + Sync + 'static {
    ...
    /// `max(source_bytes, output_bytes)`; HEAD and DELETE report zeroes.
